...
namespace Visyn.Windows.Io.FileHelper.Converters
{
    ...
    internal static class ConvertHelpers
    {
        ...
        internal abstract class CultureConverter : ConverterBase
        {
            ...
            public sealed override string FieldToString(object field) => ((IConvertible) field)?.ToString(mCulture) ?? string.Empty;
            ...
            public sealed override object StringToField(string from) => ParseString(@from);
        ...
        internal sealed class GuidConverter : ConverterBase
        {
            ...
            public override object StringToField(string from)
            {
                if (string.IsNullOrEmpty(from)) return Guid.Empty;
            ...
            public override string FieldToString(object guid) => ((Guid?) guid)?.ToString(mFormat) ?? string.Empty;
        }
